...
定投计算使用**扩展内部收益率（XIRR）**算法计算年化收益率：

$$
\sum_{k=1}^{K} \frac{CF_k}{(1 + r)^{\frac{\Delta t_k}{365}}} = 0
$$

**说明**：
- `CF_k` 表示第 `k` 笔现金流（初始投资和定期投资为负数，最终金额为正数）
- `r` 表示年化收益率（需要求解的未知数）
- `\Delta t_k` 表示第 `k` 笔现金流距离第一笔现金流的天数
- 该方程使用牛顿-拉夫森迭代法求解
- 支持多种投资频率：每天、每周、每两周、每月、每季度、每年
- 系统会根据投资频率自动计算定期投资的日期和金额
